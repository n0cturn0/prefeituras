<?php

namespace App\Http\Requests\Ldo;

use Illuminate\Foundation\Http\FormRequest;

class StoreLdoRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true;
    }

    public function rules(): array
    {
        return [
            'municipio_id' => ['nullable', 'integer'],
            'ano' => ['required', 'digits:4', 'integer', 'unique:ldos,ano'],
            'data_envio_legislativo' => ['nullable', 'date'],
            'data_devolucao_executivo' => ['nullable', 'date', 'after_or_equal:data_envio_legislativo'],
            'pdf_lei' => ['nullable', 'file', 'mimes:pdf', 'max:15360'],
            'ementa' => ['required', 'string', 'max:3000'],
            'status' => ['required', 'in:em_elaboracao,enviado,aprovado,arquivado'],
        ];
    }

    public function messages(): array
    {
        return [
            'ano.unique' => 'Já existe uma LDO cadastrada para este ano.',
            'ano.required' => 'O ano de referência da LDO é obrigatório (Art. 4º LRF).',
            'ementa.required' => 'A ementa da lei é obrigatória.',
            'data_devolucao_executivo.after_or_equal' => 'A data de devolução deve ser posterior à data de envio.',
        ];
    }
}
